...
//	width와 height를 각각 지정하면 지정된 크기만큼 수정된다.
...
//	$('img').css('width', '100px').$('img').css('height', '300px');
//	$('img').css({'width' : '100px'}, {'height' : '300px'});
	$('img').css({'width' : '100px'}, {'height' : '300px'}).css('opacity', '0.5');
}
...
//	jQuery
	...
	$('#list > li').last().css('background-color', 'yellowgreen');
	$('#list > li').eq(1).css('background-color', 'skyblue');
	$('#list > li').slice(2, 5).css('background-color', 'tomato');
}
...
//	$('img').first().css('display', 'none');	=> 	img 맨 처음꺼	
//	$('img').first().hide();	
//	$('img').first().hide('slow');	
//	$('img').first().hide('fast');	
//	$('img').first().hide(5000);
